...
test("adds the login token only for protected API URLs", () => {
  assert.equal(
    buildApiUrl("http://127.0.0.1:3010/api/v1", "/media-assets/photo/content", "http://127.0.0.1:5183", "admin-token"),
    "http://127.0.0.1:3010/api/v1/media-assets/photo/content?token=admin-token",
  );
  assert.equal(
