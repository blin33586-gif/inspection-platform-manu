...
export class ManagedObjectDeletionService {
  ...
  async reviewDeletion(auditId: string, decision: DeletionDecision, actor = "曲阳路街道管理员") {
    ...
    return this.database.$transaction(async (transaction) => {
      ...
      const request = await database.auditLog.findUnique({ where: { id: auditId } });
      ...
      const object = await database.managedObject.findUnique({ where: { id: request.targetId } });
      ...
      await database.issue.updateMany({ where: { objectId: object.id }, data: { objectId: null } });
      await database.inspectionReport.updateMany({ where: { relatedObjectId: object.id }, data: { relatedObjectId: null } });
      ...
      await database.managedObject.delete({ where: { id: object.id } });
      return database.auditLog.update({
        where: { id: auditId },
